        ...
        <>
            <div className="h-[40vh] w-[200px] bg-center bg-cover rounded-xl hover:scale-110 duration-300 hover:cursor-pointer flex flex-col justify-between items-end"
                ...
                {doesContain(movieObj) ? (
                    <div onClick={() => removeFromWatchList(movieObj)} className="m-4 flex justify-center h-8 w-8 items-center rounded-lg bg-red-900/60">❌</div>
                ) : (
                    <div onClick={() => addToWatchList(movieObj)} className="m-4 flex justify-center h-8 w-8 items-center rounded-lg bg-gray-900/60">❤️</div>


                )}

                <div className=" text-white w-full text-center text-2xl p-2 rounded bg-gray-800"> {movieObj.title}</div>
            </div>
        </>
    );
}
